...
readString(const Block& block)
{
  ...
  //return std::string(reinterpret_cast<const char*>(block.value_begin()), reinterpret_cast<const char*>(block.value_end()))
}
...
{
  EncodingEstimator estimator;
  size_t totalLength = estimator.prependByteArrayBlock(type, value, length);  //只是预先统计数量

  EncodingBuffer encoder(totalLength, 0); //后面的预留空间为0的encoder
