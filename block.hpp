...
#ifndef NDN_ENCODING_BLOCK_HPP //宏定义前先测试是否被定义过
...
{
public:
  typedef std::vector<Block>                 element_container; //别名数据结构为Block的动态数组
...
public: // constructor, creation, assignment
  ...
  /** @brief Create a Block from the wire buffer (no parsing) //what is wire buffer?
   *
   ...
   *  to construct wire encoding, one need to prepend the wire buffer with type //预设？
   *  and value-length VAR-NUMBERs
   */
  Block(uint32_t type, const ConstBufferPtr& value);

  /** @brief Create a nested Block of a specific type with the specified value //嵌套
   *
   *  The underlying buffer holds only value. Additional operations are needed
  ...
  /** @deprecated use fromBuffer(ConstBufferPtr, size_t) //警告、不建议使用
   */
  DEPRECATED(
  static bool
  ...
  /** @brief Try to construct block from raw buffer  //****what is a raw buffer?
...
public: // wire format
  ...
  /** @brief Check if the Block has fully encoded wire  //*****encoded wire is what?
   */
  bool
  hasWire() const;
