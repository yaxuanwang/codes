...
/**
 * @brief Helper class template to create a data block when RandomAccessIterator is used //可进行算术运算
 */
...
/**
 * @brief Helper class template to create a data block when generic InputIterator is used  //泛型输入指针，单向读取和遍历
 */
template<class Iterator>
class DataBlockSlow
{
...
makeBinaryBlock(uint32_t type, Iterator first, Iterator last)
{
  ...
  typedef typename boost::mpl::if_<                                                 //return dataBlockfast or datablockslow depending on is_base_of
